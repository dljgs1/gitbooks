...
通过限制每个模型副本仅在每个<img src="http://latex.codecogs.com/gif.latex?n_{fetch}" />步骤请求更新的参数，
并且仅在每个<img src="http://latex.codecogs.com/gif.latex?n_{push}" />步骤（其中<img src="http://latex.codecogs.com/gif.latex?n_{fetch}" />可能不等于<img src="http://latex.codecogs.com/gif.latex?n_{push}" />）时发送更新的梯度值，
可以减少 Downpour SGD的通信开销。
实际上，取三个参数，推梯度和处理训练数据的过程可以在三个只有弱同步的线程中进行（参见伪码附录）。
在下面报告的实验中，为了简单和易于与传统SGD进行比较，我们修正了：<img src="http://latex.codecogs.com/gif.latex?n_{fetch}=n_{push}=1" />。
