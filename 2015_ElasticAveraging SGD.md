...
其中x是要估计的模型参数，ξ是随机变量，它服从概率分布P在Ω上使得：$F(x) = \int_{\Pi}f(x,\xi)\mathbb{P}(d\xi)$。
（1）式中的优化问题可以重新表述如下：

$\mathop{min}\limits_{x^1,..,x^p,\overline{x}}\sum_{i=1}^{p}\mathbb{E}[f(x^i,\xi^i)]+\frac{\rho}{2}||x^i-\widetilde{x}||^2$

其中每个$\xi^i$服从同样的分布P（因此我们假定每个工人都可以对整个数据集进行采样）。
在本文中，我们将$\x^i$称为局部变量，我们将$\widetilde{x}$称为中心变量。
...
EASGD的更新方式见式（3）和式（4），是通过对式（2）中的目标对变量$x_i$和$\widetilde{x}$进行梯度下降步骤而获得的。

$x_{i+1}^i=x_t^i-\eta(g_t^i(x_t^i))+\rho(x_t^i-\widetilde{x}_t)$（3）

$\widetilde{x}_{t+1}=\widetilde{x}_t+\eta\sum_{i=1}^p\rho(x_t^i-\widetilde{x}_t)$（4）

其中$g_t^i(x_t^i)$表示在迭代t时F对$x^i$的随机梯度，$x_t^i$和$\widetilde{x}_t$分别表示迭代t时变量$x_i$和$\widetilde{x}_t$的值，η是学习率。

中心变量$\widetilde{x}$的更新规则采用移动平均的形式，其中平均值被空间和时间所占据。
记α=ηρ，β=pα，则式（3）和式（4）变成：

$x_{i+1}^i=x_t^i-\eta(g_t^i(x_t^i))-\alpha(x_t^i-\widetilde{x}_t)$（5）

$\widetilde{x}_{t+1}=(1-\beta)\widetilde{x}_t+\beta(\frac{1}{p}\sum_{i=1}^px_t^i)$（6）

注意，选择β=pα导致更新规则中的弹性对称性，
即在每个$x^i$和$\widetilde{x}$的更新之间存在等于$\alpha(x_t^i-\widetilde{x}_t)$的对称力。
它对算法的稳定性有至关重要的影响，这一点将在第4节中解释。
为了尽量减少中心和局部变量之间的差异、$x_t^i-\widetilde{x}_t$的陈旧性[16]，等式4涉及$x_t^i$而不是$x_{t+1}^i$

还要注意α=ηρ，其中ρ的大小代表我们在模型中允许的勘探量。
特别是，小ρ允许更多的探索，因为它允许 $x_i$ 从中心 $\widetilde{x}$ 进一步波动。
...
本地工作者仍然负责更新局部变量$x^i$，而主机正在更新中心变量 $\widetilde{x}$ 。
每个工作人员维护自己的时钟 $t^i$ ，从0开始，在xi的每个随机梯度更新之后递增1，如算法1所示。
每当本地工作者完成其梯度更新的τ个步骤时，主机执行更新，以τ为通信周期。
从算法1可以看出，每当τ划分第i个工作者的本地时钟时，第i个工人与主机通信并请求中心变量x的当前值。
然后，工作人员等待主机发回请求的参数值，然后计算算法1）中的弹性差值$\alpha(x-\widetilde{x})$
（在步骤a中捕获整个过程）。
然后，弹性差异被发回到算法1）中的主机（步骤b），然后更新 $\widetilde{x}$。
...
$v_{t+1}^i=\delta{v}_t^i-\eta{g}_t^i(x_t^i+\delta{v_t^i})$

$x_{t+1}^i=x_t^i+v_{t+1}^i-\eta\rho(x_t^i-\widetilde{x})$

其中$\eta$是动量项，注意当其为0时恢复到原本的EASGD算法。
...
设t≥0是一个全局时钟。在每个t处，我们线性化函数$F(x^i)$和$F(x^i_t)+<\nabla{F(x_t^i)},-x^i-x^i_t>+\frac{1}{2\eta}||x^i-x_t^i||^2 $
，如[28]所示。更新变为：

![](img/easgd/eq9.png)

每个局部变量$x^i$周期性地更新（周期为p）。
...
我们将拉格朗日乘子重新参数化为$\lambda_t^i\leftarrow\lambda_t^i/rho$
...
现在我们将重点讨论没有噪声的一维二次方案，即$F(x)=\frac{x^2}{2},x\in{\mathbb{R}}$

对于ADMM算法，让t时刻（动态）系统的状态为：$s_t=(\lambda_t^1,...,\lambda_t^p,x_t^p,\widetilde{x}_t)\in\mathbb{R}^{2p+1}$
...
图1：线性映射 ![](img/easgd/f_3.png) 的最大绝对特征值，$\eta\in(0,10^{-2})$，$rho\in(0,10)$，p=3和p=8的情况。
为了模拟ADMM算法的混沌行为，可以选择η= 0.001和ρ= 2.5，并随机或者以$λ_i^0 = 0,x^i_0=\widetilde{x}=1000$来初始化状态s0。
图应该用彩色阅读。

另一方面，由于弹性，EASGD算法仅包含对称的线性映射。
令t时刻（动态）系统的状态为$s_t=(x_t^1,...,x_t^p,\widetilde{x}_t)\in\mathbb{R}^{p+1}$
式（12）中激活的本地工作者i的更新和式（13）中的主要更新可以写为$s_{t+1}= F^i(s_t)$。
在p = 2的情况下，映射$F^1$和$F^2$定义如下:

![](img/easgd/f_4.png)

对于复合图Fp◦...◦F1的稳定性，每个i需要满足的条件实际上是相同的，而且与p无关（因为每个线性映射F i是对称的）。
它基本上涉及2×2矩阵的稳定性：![](img/easgd/mat.png) 
其中两个（实数）特征值λ满足$(1-\eta-\alpha-\lambda)(1-\alpha-\lambda)=\alpha^2$。
由此产生的稳定性条件（$|\lambda|\le1$）很简单，给出$0\le\eta\le2,0\le\alpha\le\frac{4-2\eta}{4-\eta}$。
...
$z_{t+1}=(1-\alpha_{t+1})z_t+\alpha_{t+1}\widetilde{x}_t$，$\alpha_{t+1}=\frac{1}{t+1}$是移动速率，
$z_0=\widetilde{x}$。
t表示主时钟，初始化为0，并在每次更新中心变量x时增加。

- 一个我们称之为MVADOWNPOUR的方法，其中我们计算中心变量x的移动平均值如下：
$z_{t+1}=(1-\alpha)z_t+\alpha\widetilde{x}_t$移动速率α选择为常数，$z_0=\widetilde{x}$。
...
- 具有恒定的学习率η的SGD [1]
- 动量δ不变的动量新元（MSGD）。
- 移动速度为$\alpha_{t+1}=\frac{1}{t+1}$的ASGD
- 移动速度α常数的MVASGD
