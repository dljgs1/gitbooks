## 摘要
机器学习（ML）正在成为一种商品。许多ML框架和服务可用于不是ML专家的数据持有者，但希望对他们的数据进行预测模型的训练。训练敏感输入（例如，个人图像或文档）的ML模型不会泄漏关于训练数据的太多信息，这一点很重要。

我们考虑一个恶意的ML提供者向数据持有者提供模型训练代码，不观察训练，然后获得对所得模型的白箱或黑箱访问。在这个背景下，我们设计和实现了一些实际的算法，其中一些与正则化和数据增强（data augmentation）等标准ML技术非常相似，能够“记忆”模型中训练数据集的信息，但是模型和一个准确和预测性一样传统的训练模式。然后我们解释对手如何从模型中提取记忆的信息。

我们评估了我们用于图像分类（CIFAR10），脸部识别（LFW和FaceScrub）以及文本分析（20个新闻组和IMDB）的标准ML任务的技术。在所有情况下，我们都会展示我们的算法如何创建具有高预测能力的模型，并准确提取其训练数据的子集。
...
在白盒子案例中，我们评估了几个技术：
1）将敏感信息编码到模型参数中最不重要的部分
2）迫使参数与敏感信息高度相关；
3）在参数符号中编码敏感信息。后两个技术包括添加恶意的“正则化”项目到损失函数中，而在数据持有者角度看上去像另一个正则化技术。
...
我们关注的是在监督学习环境下的简单性，但是我们的技术也可以应用于非监督学习。机器学习模型学习的是函数fθ:X→Y,由字符串参数θ参数化。
...
客户将对手提供的机器学习管道（参见图1）应用到Dtrain，训练子集这个管道输出一个模型，由它的参数θ定义。客户端通过测量其在测试子集Dtest和测试列表间的准确性来验证模型，如果通过验证则接受该模型，然后通过θ或者fθ的API接口预测查询来发布它。我们将前者称为白盒访问，后者称为黑盒访问模型。
...
一些现有的模型已经揭示了部分训练数据。
例如，最近的邻居分类器和支持向量机（SVM）明确地将一些训练数据点存储在θ中。深度神经网络和经典的逻辑回归不知道泄漏任何特定的训练信息（参见第8节关于现有训练算法的隐私的更多讨论）。即使使用SVM，攻击者也可能想要在默认设置下提供更多或不同的训练数据。对于攻击者无法直接访问θ的黑盒攻击，目前还没有一种方法通过SVM和最近邻模型来提取存储在θ中的敏感数据。
